...
\begin{defn}[Cartier dual]
	...
	as the group scheme
	\begin{equation}
		G' \coloneqq \mathcal{H}\mathrm{om}_{\mathsf{Gr}_S} \left( G, (\mathbb{G}_m)_S \right)
	.\end{equation} 
\end{defn}


\begin{rem}[Affine case]
	In the affine case one can easily check that, for $S = \mathrm{Spec}(R)$
	and $G = \mathrm{Spec}(A)$, the Cartier dual $G'$ is given by
	the $R$-algebra 
	\begin{equation}
	A' \coloneqq \mathrm{Hom}_{R\text{-}\mathsf{Mod}} \left( A, R \right)
	.\end{equation} 
	...
	coproduct and product of $A'$, by tk.
\end{rem}


The following will be a particularly relevant definition in what follows:
\begin{defn}[Finite $S$-group]
	For $S = \mathrm{Spec}(R)$, we say that an $S$-group $G$
	is finite of order $m$ over $R$ iff it is locally free of rank $m$ over $R$.
	In other words if it is defined by a locally free sheaf of algebras of rank $m$
	over $R$.
\end{defn}
\end{document}
