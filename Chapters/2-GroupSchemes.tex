...
\begin{rem}[]
	As of \cref{rem:finiteGSMotivation} $A$ is a finite projective $R$-module
	(hence, still by 
	\cite[\href{https://stacks.math.columbia.edu/tag/00NX}{Lemma 00NX}]{SP}),
	$A$ is a finite and locally free $R$-module).
	For an $R$-module $M$ we define its dual $R$-module to be
	$\mathrm{Hom}_{R} \left( M, R \right)$.
	Then, as seen in \cite[\S 3.8]{TateMF}, for any locally free
	$R$-modules $M$ and $N$, one has the natural isomorphisms
	\begin{equation}
	\begin{tikzcd}
