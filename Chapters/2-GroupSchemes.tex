...
%				\mathsf{Gp} \\
		...
		\item The {\em constant group scheme}\label{ex:ConstantGroups}.
			Let $R$ be a ring and $\Gamma$ a finite commutative (usual) group.
			...
			for $e_\sigma(\gamma) = \delta_{\gamma,\sigma}$, for the Kronecker delta.
			Defining the Hopf algebra morphisms by
			\begin{equation*}
				...
				\end{cases} 
			\end{equation*}
			we see that $A$ represents an $S = \mathrm{Spec}(R)$ group scheme,
			called the constant group scheme for $\Gamma$,
			and we denote it by $\underline{\Gamma}$.
...
Let's introduce the relevant notions:
...
\begin{rem}[Dual as hom functor, \cite{Waterhouse}, \textsection $2.4$]
	...
	.\end{tikzcd}
	\end{equation*} 
	Then, for a finite, commutative $S$-group $S$ we have the isomorphism
	\begin{equation*}
...
\begin{enumerate}
	\item The dual algebra of $R^\Gamma$ is $R[\Gamma]$ and viceversa.
		Hence diagonalizable group schemes are dual to constant group schemes.
