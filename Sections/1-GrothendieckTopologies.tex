...
inclusions with certain families of arrows in a category and 
...
\begin{rem}[Open subsets of a topological space]
	...
	In fact all arrows in $\mathsf{Op}(X)$ correspond to inclusions and
...
\begin{rem}[]
	...
	and $\left\{ V_{ j } \to U \right\}_{ j \in J }$, also
...
\noindent
Before moving on to some examples, let's introduce a
...
\begin{rem}[]\label{CharacterizationSheaves}
...
\begin{rem}[]
	...
	any sheaf in the fpqc topology is also a sheaf in the fppf, étale and Zariski topologies.
\end{rem}
...
\begin{defn}[Topoi]
	...
	and $f^{-1}\colon \mathsf{Sh}\left(\mathsf{C}\right) \to \mathsf{Sh}\left(\mathsf{D}\right)$
	such that, bifunctorially we have
	\begin{equation*}
		\Hom_{\mathsf{Sh}(\mathsf{D})} \left( f^{-1}G, F \right) \simeq
		\Hom_{\mathsf{Sh}(\mathsf{C})} \left( G, f_*F \right) 
	\end{equation*}
	and the functor $f^{-1}$ commutes with finite limits.
	%, i.e. it is left exact.
	...
	and $\left( f \circ g \right)^{-1} \coloneqq g^{-1} \circ f^{-1}$.
\end{defn}
...
\begin{defn}[Category over an object]
	...
	$Y \to U$ in $\mathsf{C}$ and whose morphisms are morphisms $Y \to Y'$
...
\begin{rem}[]
	...
	Moreover there is a forgetful functor $p_U\colon \mathsf{C}/U \to \mathsf{C}$
	...
	by composition with $f$.
	Moreover we can notice that $p_U = p_V \circ F$.
\end{rem}
...
\begin{lem}[{\cite[\href{https://stacks.math.columbia.edu/tag/00XZ}{Section 00XZ}]{SP}}]
	...
	$p_U\colon \mathsf{C}/U \to \mathsf{C}$ induces a
	morphism of topoi
	\begin{equation*}
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
	...
	.\end{tikzcd}
	\end{equation*} 
	given by the functors $j_U^{-1}$ and ${j_U}_*$, whose lengthy definition
...
\begin{enumerate}
	...
	\item Taken any sheaf $F \in \mathsf{Sh}\left(\mathsf{C}\right)$,
		its image $j_U^{-1}F$ is called the \emph{restriction} of $F$ to $\mathsf{C}/U$.
\end{enumerate}
\end{defn}
...
\begin{rem}[{\cite[\href{https://stacks.math.columbia.edu/tag/00XZ}{Section 00XZ}]{SP}}]
	...
	For all $F \in \mathsf{Sh}\left(\mathsf{C}\right)$, the evaluation of $j_U^{-1}F$
	on \(X/U \coloneqq X \to U \in \mathsf{C}/U\) by
	\begin{equation*}
		j_U^{-1}F(X/U) = F(X)
	.\end{equation*}
\end{rem}
...
\begin{lem}[{\cite[\href{https://stacks.math.columbia.edu/tag/03I4}{Lemma 03I4}]{SP}}]
	...
	%, viewed as a site with the fppf topology
...
\begin{enumerate}
	\item $j^{-1} F'(T/S) = F'(T/S')$ for any $T \in \mathsf{Sch}/S$,
		where $T/S'$ is the image of \(T/S\) after composition with \(g\).
	\item if $F'$ is representable by $X' \in \mathsf{Sch}/S'$,
		then $j^{-1}F'$ is representable by $X'_S \coloneqq X' \times_{ S' } S$.
\end{enumerate}
\end{lem} 
