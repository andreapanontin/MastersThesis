...
\begin{rem}[fpqc topology]
	...
	s.t. the induced morphism $\amalg_i U_i \to U$ is \emph{fpqc},
...
\noindent
Notice that any \emph{sheaf} on $\mathsf{C}$ is also separated.
...
\begin{rem}[]
	...
	any sheaf in \emph{fpqc} is also a sheaf in \emph{fppf}, étale and Zariski topologies.
\end{rem}
...
\begin{thm}[{\cite[\S 2.3.6]{Vistoli}}, Grothendieck]\label{thm:reprFctSheaf}
	A representable functor on $\mathsf{Sch}/S$ is a sheaf in the \emph{fpqc} topology.
	In particular it is also a sheaf in the étale and \emph{fppf} topologies.
\end{thm}
...
\begin{enumerate}
	...
	\item Taken any sheaf $F \in \mathsf{Sh}\left(\mathsf{C}\right)$,
		its image $j_U^*$ is called the \emph{restriction} of $F$ to $\mathsf{C}/U$.
\end{enumerate}
\end{defn}
