...
\begin{rem}[$T$-points of an $S$-scheme]
	...
	.\end{equation*} 
	If we view $G$, by \cref{reprFctSheaf}, as an {\em fppf} sheaf on $\mathsf{Sch}_{ S }$, instead,
...
\begin{rem}[]\label{StructuralMorphisms}
...
of schemes defined in \cref{StructuralMorphisms} into
appropriate $R$-algebra morphisms.
Then the properties defined by the diagrams in \cref{StructuralMorphisms}
...
\begin{equation*}
 ...
 ,/tikz/column 3/.append style={anchor=base east}]
	...
	\end{tikzcd}
\end{equation*} 
satisfying the duals of the diagrams in \cref{StructuralMorphisms}.
\end{rem}
...
\begin{defn}[Hopf algebras]\label{defn:HopfAlgebra}
	...
	of \cref{StructuralMorphisms}, more explicitly:
	\begin{align*}
...
\begin{rem}[Finite $S$-scheme]\label{finiteGSMotivation}
...
\begin{rem}\label{ExplicitComult}
...
\begin{ex}
	...
	\begin{enumerate}
		\item The {\em additive group scheme}, given by 
			...
			.\end{equation*} 
			Then, thanks to \cref{ExplicitComult},
			we desume that
			\begin{equation*}
				\widetilde{m}(x) =
...
\begin{prop}[{\cite[\S 3.5]{TateMF}}]\label{QuotientGroupScheme}
...
\begin{rem}[Short exact sequence]
	Assume, in the hypothesis of \cref{QuotientGroupScheme}, that $H$ is
	also {\em normal} in $G$.
...
\begin{thm}[{\cite[\S 6.2]{Waterhouse}}]
	\label{SeparableAlgebrasCharacterization}
...
\begin{defn}[Separable algebra]
	...
	\cref{SeparableAlgebrasCharacterization}
...
\begin{thm}[{\cite[\S 3.7]{TateMF}}, Connected-étale exact sequence]\label{ConnecteEtaleSequenceGS}
	...
	constructed as in \cref{QuotientGroupScheme},
...
\begin{prop}[{\cite[\S 3.7]{TateMF}}]\label{prop:PropertiesConnecteEtale}
\leavevmode\vspace{-.2\baselineskip}
\begin{enumerate}
...
\begin{rem}[]
	In \cref{finiteGSMotivation} we recalled that $A$ is a finite projective $R$-module.
...
\begin{thm}[{\cite[\S 3.7]{Waterhouse}}, Cartier duality]\label{CartierDuality}
...
\begin{ntt}
	...
	we define 
	%the morphism $f^{(p)}$ as the morphism
	\begin{equation}
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
		,/tikz/column 2/.append style={anchor=base west}]
...
\begin{prop}[]\label{FrobeniusChar1}
...
\begin{prop}[]\label{FrobeniusChar2}
...
\begin{defn}[Verschiebung]
	Let $G$ be a finite, commutative $R$-group scheme.
	By \cref{CartierDuality} $(G^{(p)})^D \simeq (G^D)^{(p)}$.
...
\cref{FrobeniusChar1} and \cref{FrobeniusChar2}.
\begin{prop}[]
...
\begin{thm}[{\cite[\S11.i]{MilneAG}}]\label{FrobeniusVerschiebungRelation}
