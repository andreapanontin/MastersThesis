...
\begin{rem}[$T$-points of an $S$-scheme]
	...
	.\end{equation*} 
	If we view $G$, by \cref{thm:reprFctSheaf}, as an \emph{fppf} sheaf on $\mathsf{Sch}/S$, instead,
...
\begin{defn}[Hopf algebras]\label{defn:HopfAlgebra}
	An \emph{Hopf algebra over $R$} is an $R$-algebra $A$,
...
\begin{defn}[Normal subgroup]
	...
	We say that $H$ is a \emph{normal subgroup} of $G$ iff, for all $T \in \mathsf{Sch}/S$,
	the subgroup $H(T)$ of $G(T)$ is a \emph{normal} subgroup.
\end{defn}
...
\begin{defn}[Discrete $\mathscr{G}_k$ groups]
	...
	We define the category of \emph{discrete $\mathscr{G}_k$ groups} as the category
