...
Then, for most of the following section we will fix an affine scheme
...
\begin{defn}[Hopf algebras]\label{defn:HopfAlgebra}
	...
	endowed with a comultiplication, a counit and an antipode map,
	respectively:
	\begin{equation*}
			\begin{tikzcd}[row sep = 0ex
         ,/tikz/column 1/.append style={anchor=base east}
...
the assumption for $S$: we will only assume it is a locally Noetherian scheme.
...
\begin{rem}[Base change]\leavevmode\vspace{-0.2\baselineskip}
\begin{enumerate}
...
\begin{rem}[{\cite[\S3, example (7)]{Shatz}}]
...
\leavevmode\vspace{-.2\baselineskip}
\begin{enumerate}
	\item Assume $\mathrm{char}\, R = 0$, then $G^0 = S$
		and $G = G^{\mathrm{\acute et}}$.
		If, instead, $\mathrm{char}\, R = p > 0$, we have
...
In this section we will introduce a notion of duality for 
finite, flat and commutative group schemes over a ring.
...
\begin{defn}[Verschiebung]
	...
	The word Verschiebung is a German word, meaning "the shift".
\end{defn}
