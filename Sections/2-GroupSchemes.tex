...
\begin{rem}[]
	...
	\begin{align}\label{eq:GroupAxioms}
		m \circ (\mathrm{id}_{ G } \times m) &= m \circ (m \times \mathrm{id}_{ G }),\nonumber\\
		m \circ (\mathrm{id}_{ G } \times \mathrm{inv}) \circ \Delta &=
		m \circ (\mathrm{inv} \times \mathrm{id}_{ G }) \circ \Delta = \varepsilon \circ \pi,\\
		m \circ (\varepsilon \times \mathrm{id}_{ G }) &=
		m \circ (\mathrm{id}_{ G } \times \varepsilon) = \mathrm{id}_{ G }\nonumber
	.\end{align} 
	...
	\begin{enumerate}
		\item Associativity of the product
			...
			\begin{tikzcd}[column sep=4.5em]
				...
				\arrow[r, "\mathrm{id}_{ G } \times_S m", rightarrow] 
				\arrow[d, "m \times_S \mathrm{id}_{ G }"', rightarrow] &
		...
		\item Inverse morphism 
			...
			\begin{tikzcd}[column sep=2.7em]
				G \times_{ S } G
				\arrow[rr, "\mathrm{id}_{ G } \times_S \mathrm{inv}", rightarrow] & &
				G \times_{ S } G \arrow[d, "m", rightarrow] \\
			...
			\begin{tikzcd}[column sep=2.7em]
				G \times_{ S } G
				\arrow[rr, "\mathrm{inv} \times_S \mathrm{id}_{ G }", rightarrow] & &
				G \times_{ S } G \arrow[d, "m", rightarrow] \\
				G \arrow[u, "\Delta", rightarrow] 
				\arrow[r, "\pi", rightarrow] &
				S \arrow[r, "\varepsilon", rightarrow] & 
		...
		\item Identity element (again, as with groups, in the following diagrams
			...
			\begin{tikzcd}
				...
				\arrow[u, "\varepsilon \times_S \mathrm{id}_{ G }", rightarrow] &
			...
			\begin{tikzcd}
				...
				\arrow[u, "\mathrm{id}_{ G } \times_S \varepsilon", rightarrow] &
				G. \arrow[u, "\mathrm{id}_{ G }"', rightarrow] 
			\end{tikzcd}
			\end{equation*} 
	\end{enumerate}
	...
	\begin{enumerate}
		...
		\item The {\em diagonalizable group schemes}. Let $X$ be an ordinary
			...
			\begin{equation*}
				...
				\mathrm{Hom}_{\mathsf{Ab}} \left( X, A^{\times} \right)
			,\end{equation*} 
...
\begin{lem}[{\cite[\href{https://stacks.math.columbia.edu/tag/02GM}{Lemma 02GM}]{SP}}]
	...
	X \times_{\mathrm{Spec}(\kappa(s))} \mathrm{Spec}(\kappa(s))$
...
\end{enumerate}
	...
	\begin{equation}
	\begin{tikzcd}
		X^{(p)} \coloneqq X \times_{ R } \mathrm{Spec}(R) \arrow[r, "", rightarrow] 
		\arrow[d, "", rightarrow] &
...
\begin{ntt}
	...
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
		,/tikz/column 2/.append style={anchor=base west}]
		f^{(p)} \coloneqq f \times \mathrm{id}_{ R }\colon 
		X^{(p)} \arrow[r, "", rightarrow] &
		Y^{(p)}
	.\end{tikzcd}
	\end{equation} 
