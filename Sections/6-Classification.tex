...
\begin{rem}[]
	...
	In the following we will want to work over the base scheme
...
\begin{ex}[]
	...
	Then, still by \cref{rem:DGValuationRestriction}, we see that $M(G)$
	...
	Moreover, when determining the filtration, it is useful to keep
...
\begin{enumerate}
	\item Let's start with $G = \varinjlim \mu_{p^v}$ and denote $G_v \coloneqq \mu_{p^v}$.
		...
		$O_K$ and it is given by
		\begin{equation*}
			\mu_{p^v} = \mathrm{Spec}(O_K[T]/ (T^{p^v} - 1)) = \mathrm{Spec}(A_v)
		.\end{equation*}
		...
		\begin{tikzcd}[row sep = 0ex
			,/tikz/column 1/.append style={anchor=base east}
			,/tikz/column 2/.append style={anchor=base west}]
			\widetilde{\epsilon}\colon 
			O_K[T]/ (T^{p^v} - 1)\arrow[r, "", rightarrow] &
			O_K
		.\end{tikzcd}
		\end{equation*} 
		Then, denoting by $I$ the augmentation ideal of $A_v$,
		the sheaf $\omega_{G_v}$ is the $\mathcal{O}_{ X }$-module
		...
		\begin{equation*}
			\Omega^1_{A_v/O_K} = \frac{ A_v \mathrm{d}T }{ A_v p^v T^{p^{v-1}} \mathrm{d}T }
			= A_v / (p^v) \frac{ \mathrm{d}T }{ T }
		.\end{equation*}
		Then, since $\widetilde{\epsilon}$ is an $O_K$-algebra morphism
		(hence it admits a right inverse) we can invoke
		\cite[\href{https://stacks.math.columbia.edu/tag/02HP}{Lemma 02HP}]{SP}
		with $S' = R = O_K$ and $S = A_v$.

		tk: maybe it is better to invoke
		\cite[\href{https://stacks.math.columbia.edu/tag/0474}{Lemma 0474}]{SP}
		and conclude with this.
		This grants that
		\begin{equation*}
		I/I^2 = \Omega^1_{A_v/O_K} \otimes_{O_K} O_K =
		O_K
		.\end{equation*}
	...
	\item Let's now analyze $G^D = \varinjlim \underline{\mathbb{Z}/p^v\mathbb{Z}}$, where
		...
		As seen in \cref{EtaleConstantGroupScheme} this is an étale
		group scheme over $O_K$.
		But then \cite[\href{https://stacks.math.columbia.edu/tag/00U0}{Section 00U0}]{SP}
		grants that $\Omega_{G_v^D/O_K} = 0$
		and finally that $\omega_{G_v^D} = \Omega_{G^D_v/O_K} \otimes_{O_K} O_K = 0$ thanks to 
		\cite[\href{https://stacks.math.columbia.edu/tag/02HP}{Lemma 02HP}]{SP}
		(here we take $R = S' = O_K$ and $S = A_v^D$, where $G_v^D = \mathrm{Spec}\, A_v^D$).
		Then, taking projective limits, we obtain $\omega_{G^D} = 0$.
\end{enumerate}
	The above computations hold also for our lift $\widetilde{G} \in \mathsf{BT}(S)$. 
	tk: why is $G^D$ still etale?
\end{ex}
