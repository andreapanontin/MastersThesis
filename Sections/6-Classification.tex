...
\item The inverse to the second bijection has the following form.
	...
	Then the induced map on $\mathcal{B}$ acts by
	\begin{equation*}
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
		,/tikz/column 2/.append style={anchor=base west}]
...
\begin{rem}[]\label{rem:A2Kisin}
	Even though we will not prove the theorem, we feel it is important to report
...
to $\mathcal{D}_{W[u]}(E(u))$ as the unique map making the diagram commute
\begin{equation*}
\begin{tikzcd}
	&
	\mathcal{D}_{W[u]}(E(u))
...
$S/pS = \mathcal{D}_{W[u]}(E(u))/ (p) = W[u]/ (p)$.
...
is definitely more tricky and we will only concentrate on
