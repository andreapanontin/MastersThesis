...
\begin{defn}[Pseudo-discrete sheaves]
	A sheaf $\mathcal{F}$ of topological rings (resp$.$ modules, groups, etc)
...
\begin{rem}[]
	...
	our main interest.
	Given an admissible ring $\mathscr{A}$, with fundamental system
...
\begin{rem}[]\label{rem:VeryFiniteSchemes}
	...
	\begin{enumerate}
		\item $T$ is \emph{finite} over $S$ (hence affine) and
...
%	is representable iff it is \emph{left exact}, i.e.
...
%implies that one need to check \emph{right exactness} instead of left exactness.
...
%	We say that $\mathcal{F}$ is a \emph{quasi-coherent sheaf of $\mathcal{O}_{ X }$-modules}
...
\begin{defn}[Conormal sheaf of an immersion]
	...
	This last sheaf, denoted with $\omega_i$, is called the \emph{conormal
...
\item A sheaf of groups $G$ on $S$ is said to be \emph{of $F$-torsion} iff
...
\begin{defn}[$p$-divisible group]\label{defn:pDivGroupFormalSchemes}
	...
	satisfying:
	\begin{enumerate}
		\item for each $v \in \mathbb{N}$, $G_v$ is a finite,
			commutative group scheme over $\mathscr{R}$
			of order $p^{v h}$;
