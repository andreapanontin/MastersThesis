...
the notions of {\em formal Lie group}
and of {\em $p$-divisible group},
...
\item We say that $R$ is {\em linearly topologized} iff $0$ has a fundamental system
...
\begin{defn}[Associated pseudo-discrete sheaf]\label{defn:AssociatePDSheaf}
	...
	in the definition of sheaf, which is comes from the analogue exact sequence
	to that in \cref{CharacterizationSheaves}.
\end{defn}
...
\item Let $A$ be an {\em admissible ring}, with fundamental system of
	...
	\text{ is open and prime}\, \right\} \subset \spec(A)
	,\end{equation*}
	and endow $\spf(A)$ with the subset topology.
	For each $\lambda \in \Lambda$, one can associate,
	as in \cref{defn:AssociatePDSheaf},
	to the structure sheaf $\mathcal{O}_{\spec(A/I_\lambda)}$
	of $\spec(A/I_\lambda)$,
	a pseudo-discrete sheaf, which we will denote by $\mathcal{O}_\lambda$.
	One should also notice that, since $A$ is admissible,
	$\spec(A/I_\lambda)$ has indeed the same topological space
	as $\spf(A)$ for all $I_\lambda$.
	Moreover, for $I_\lambda \subset I_\mu$, one has an induced homomorphism
	\begin{equation*}
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
		,/tikz/column 2/.append style={anchor=base west}]
		\spec(A/I_\mu) \arrow[r, "", rightarrow] &
		\spec(A/I_\lambda)
	,\end{tikzcd}
	\end{equation*} 
...
\begin{rem}[]\label{EquivalenceFormalAffineSchemes}
	...
	\begin{equation*}
	\Hom_{\mathsf{ltrs}} \left( \spf(B), \spf(A) \right) \simeq
	\Hom_{ \mathrm{cont} } \left( A, B \right)
	,\end{equation*}
...
\begin{rem}[]
	...
	our main interest.
	Given an admissible ring $A$, with fundamental system
	of ideals $\left\{ I_\lambda \right\}_{\lambda \in \Lambda}$, then
	we have
	\begin{equation*}
	\spf(A) \simeq \varinjlim_{\lambda \in \Lambda} \spec(A/I_\lambda)
	\end{equation*}
...
\begin{rem}[]\label{rem:VeryFiniteSchemes}
	...
	local admissible ring and $S \coloneqq \spec(R)$.
	...
	There, we read that an $S$-schemes $T$ is very finite iff it satisfies:
	\begin{enumerate}
		\item $T$ is {\em finite} over $S$ (hence affine) and
...
we fix $R$ a local admissible ring, $S \coloneqq \spec(R)$
...
\begin{ntt}\label{not:GBar}
	...
	,\end{equation*}
	where the limit is taken with respect to the obvious inclusion morphisms.
\end{ntt} 
...
\begin{defn}[Conormal sheaf of an immersion]
	...
	sheaf} of $Z$ in $X$, or the conormal sheaf of the immersion $i$.
...
%		.\end{tikzcd}
%		\end{equation*} 
%		Analogously the sheaf $\sym^n(\mathcal{F})$ is the 
...
%			,/tikz/column 2/.append style={anchor=base west}]
%			U \arrow[r, "", mapsto] &
%			\sym_{\mathcal{O}_{ X }(U)}^n (\mathcal{F}(U))
%		.\end{tikzcd}
%		\end{equation*} 
...
%		\cite[\href{https://stacks.math.columbia.edu/tag/01CF}{Section 01CF}]{SP}.
	...
	\item \label{SheafqcSymExt}
		...
		$\sym(\mathcal{F})$ are {\em quasi-coherent} (resp$.$ {\em locally-free}),
...
\begin{enumerate}
	...
	\item Denoting by $\mathrm{gr}^{\mathrm{inf}}(X)$ the unique graded $\mathcal{O}_{ S }$-algebra
		...
		%tk: if you have time define the graded algebra better.
\end{enumerate}
\end{defn}
...
%tk: a motivating question. What should push us to lose representability in the first place?
...
\item Denote by $\underline{\mathbb{Z}/p^v\mathbb{Z}}$ the base change to $R$\label{ex:Qp/Zp}
	...
	to the group $\Gamma \coloneqq \mathbb{Z}/p^v\mathbb{Z}$, see \cref{ex:ConstantGroups} of
...
\begin{rem}[{\cite[Chpater I, \S2.4.1]{Messing}}]
	...
	Moreover we call {\em lift} of $G \in \mathsf{BT}/S'$ via $f$ any 
...
i.e$.$ viewing a $p$-divisible group as an inductive system of finite commutative group schemes.
...
\begin{prop}[{\cite[\S2.3, proposition 3]{TatePC}}]
	...
	coincide and the common value satisfies
	\begin{equation*}
	h = n + n^D
	.\end{equation*}
\end{prop}
...
\begin{rem}[]\label{rem:TateModuleProperties}
...
\begin{prop}[]\label{TateModuleDuals}
