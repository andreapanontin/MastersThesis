...
\item Let $A$ be an {\em admissible ring}, with fundamental system of
	...
	as $\mathrm{Spf}(A)$ for all $I_\lambda$.
...
\begin{enumerate}
	...
	\item $\omega_X \simeq
		...
		\simeq e_X^* \big( \Omega_{\mathrm{Inf}^k(X)/S} \big)$, for any $k \in \mathbb{N}$,
...
\begin{rem}[]\label{rem:TateModuleProperties}
	...
	\begin{equation*}
		...
		\varprojlim_{v \in \mathbb{N}} \mathrm{Hom}_{ \mathsf{Sch}/O_K } 
	...
	\end{equation*}
	for all $g \in \mathscr{G}_K$, $x \in A_v$ and $f \in \mathrm{Hom}_{ O_K\text{-}\mathsf{Alg} } 
	\left( A_v, \overline{K}\right)$.
\end{rem}
