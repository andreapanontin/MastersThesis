...
\begin{rem}[]\label{constr:ComparisonMorphism}
	...
	.\end{tikzcd}
	\end{equation*} 
	But this allows one to see each element $a \in \mathbb{D}^*(G_{O_{\mathbb{C}_K}})(A_{\mathrm{cris}})$
	as the $\mathbb{Z}_{p}$-linear morphism of evaluation in $a$.
	More explicitly, to each $f \in T_p(G)$, it associates $f(a) \in A_{\mathrm{cris}}$.
	Finally this gives rise to the homomorphism
	\begin{equation*}
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
		,/tikz/column 2/.append style={anchor=base west}]
		...
		\mathbb{D}^*(G_{O_{\mathbb{C}_K}})(A_{\mathrm{cris}})
		\arrow[r, "", rightarrow] &
		A_{\mathrm{cris}} \otimes_S T_p(G)^\vee
	.\end{tikzcd}
	\end{equation*} 
\end{rem}
