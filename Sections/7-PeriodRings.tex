...
\begin{defn}[$(F,\Gamma)$-regular algebra]
	...
	We say that $B$ is {\em $(F,\Gamma)$-regular} iff
\begin{enumerate}
	\item $B^{\Gamma} = C^{\Gamma}$ and
	\item if $b \in B$ generates a vector space $F \cdot b$
		stable under the action of $\Gamma$, then $b \in B^{\cross}$.
\end{enumerate}
\end{defn}


\begin{rem}[]
	Notice that, for any $(F,\Gamma)$-regular algebra, $E/F$ is a field extension.
