...
\begin{proof}
	...
	of $p$ power roots of unity in $O_{\mathbb{C}_K}$.
	...
	evaluated at $O_{\mathbb{C}_K}$, gives rise to the following diagram
	\begin{equation*}
	\begin{tikzcd}
		0 \arrow[r, "", rightarrow] &
		O_{\mathbb{C}_K} 
...
%	$\mathscr{G}_{K_\infty}$ both sides is diagonal (especially it is trivial
%	on $\mathbb{D}^*(\mathbb{G}_m(p))(S)$), we also see that $\rho_G$
%	is $\mathscr{G}_{K_\infty}$-equivariant.
	...
	We are left to prove injectivity.
	For the first notice that, thanks to classical Dieudonné theory
...
\begin{rem}[]
	...
	the map $u \mapsto \left[ \mathbf{p} \right]$ from $W[u]$ to $W(R)$.
	...
	In particular, since $W \subset K$, it has to fix the element $\left[ \mathbf{p} \right]$,
...
\begin{thm}[]
	...
	\end{tikzcd}
	\end{equation*} 
	is a $\mathscr{G}_K$-equivariant isomorphism compatible with filtrations and Frobenius.
\end{thm}
...
\begin{rem}[]
	...
	\begin{align*}
		\mathrm{Fil}^1 \left( B_{\mathrm{dR}} \otimes_{W} D_K \right) &=
	...
	Moreover, having no natural filtration on $T_p(G)$,
	one defines
	\begin{equation*}
	...
	\end{tikzcd}
	\end{equation*} 
	is an isomorphism compatible with the filtrations we just defined.
\end{rem}
