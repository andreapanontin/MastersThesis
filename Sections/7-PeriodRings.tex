...
\begin{proof}
	...
	of all $p^n$ roots of unity in $O_{\mathbb{C}_K}$.
	...
	and \cref{lem:BT/admissible} it can shown that they are given by
	\begin{equation*}
	\begin{tikzcd}
	...
	Then \cref{thm:UniqueLiftingCrystals}, thanks to \cref{rem:NonNilpotentCrystalEvaluation}
	and the fact that $A_{\mathrm{cris}}$ is $p$-adically complete,
	...
	In fact we recall that, as computed in \cref{DieudonneModulesSimpleExamples},
	\(\mathbb{D}^*(\mathbb{G}_m(p))(S) \simeq S\).
	Then the morphism \(\rho_G\) acts by
	\begin{equation*}
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
		,/tikz/column 2/.append style={anchor=base west}]
		...
		a \otimes b \arrow[r, "", mapsto] & -ab \cdot t \otimes t^{-1}
	.\end{tikzcd}
	\end{equation*} 
...
\noindent
Inverting t, i.e$.$ base changing to $B_{\mathrm{cris}}$, the above result gives rise to
