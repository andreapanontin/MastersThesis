...
\begin{defn}[$p$-adic representation]
	...
	Let's denote, for any \(\gamma \in \Gamma\),
	the morphism $\rho_i(\gamma)$ simply by $\gamma$,.
	Then a $\Gamma$-equivariant
	map is a map that satisfies $\gamma(f(v)) = f(\gamma(v))$
...
\begin{defn}[]
	...
	We can associate to it the perfect $\mathbb{F}_p$-algebra
	\begin{equation*}
...
\item If $A$ is already perfect, then $R(A) \simeq A$.
	...
	$a \mapsto \left( a^{1/p^n} \right)_{ n }$.
...
\begin{ntt}[]
	...
	algebra over $\overline{k}$, since $O_{\mathbb{C}_K}/p O_{\mathbb{C}_K}$ is.
\end{ntt}
...
\begin{rem}[]\label{tiltingOperations}
	Above \(\varprojlim_{ x \mapsto x^p } O\) was taken in \(\mathsf{Sets}\).
	Though we can endow $\varprojlim_{x \mapsto x^p} O$ with the ring structure
	...
	.\end{equation*}
	With this extra structure the above bijection is an isomorphism of rings.
\end{rem}
...
\begin{ntt}[]\label{not:tiltingElts}
	...
	$\left( \mathbf{x}_n \right) \in \varprojlim_{\varphi} O_{\mathbb{C}_K}/p O_{\mathbb{C}_K}$
	or as an element
...
\begin{rem}[]
	...
	\begin{tikzcd}[row sep = 0ex
		...
		O_{\mathbb{C}_K}/p O_{\mathbb{C}_K} \\
		\left( \mathbf{x}_m \right)_{m \in \mathbb{N}} \arrow[r, "", mapsto] & 
		\mathbf{x}_n
	.\end{tikzcd}
	\end{equation*} 
	Let's recall the $\overline{k}$-algebra structure
	of \(R\). It is given by the $k$-embedding
	\begin{equation*}
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
		,/tikz/column 2/.append style={anchor=base west}]
...
\begin{rem}[]
	...
	\mathbf{r}^{(n)}$.
	Hence we can also compute $\theta$ via
	\begin{equation*}
	\begin{tikzcd}
		\theta\colon 
...
\begin{lem}[{\cite[Lemma 4.4.1]{Brinon}}]\label{PropertiesTheta}
	The map $\theta\colon W(R) \twoheadrightarrow O_{\mathbb{C}_K}$ is a
	surjective $\mathscr{G}_K$-equivariant ring homomorphism.
\end{lem} 
...
\begin{rem}[]\label{kerQGKStable}
	As a consequence we see that $W(R)[1/p]$
	...
	.\end{equation*}
	Since \(\theta_{ \mathbf{Q} }\) is a $\mathscr{G}_K$-equivariant morphism,
	$\ker \theta_{\mathbf{Q}}$ is stable under the action of $\mathscr{G}_K$
	...
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
		,/tikz/column 2/.append style={anchor=base west}]
		\theta^+_{\mathrm{dR}}\colon B^+_{\mathrm{dR}} 
		\arrow[r, "", twoheadrightarrow] &
		\mathbb{C}_K
	.\end{tikzcd}
	\end{equation*} 
...
\begin{enumerate}
	...
	\item Recall that, as stated in \cref{ExampleEltsTilt},
		for any two choices \(\boldsymbol\varepsilon\) and
		\(\boldsymbol\varepsilon'\) there is an \(a \in \mathbb{Z}_{p}^\times\)
		such that \(\boldsymbol\varepsilon' = \boldsymbol\varepsilon^a\).
		Let's denote by \(t' \coloneqq \log ([\boldsymbol\varepsilon'])\).
		Then it can be shown, via careful topological arguments, 
		that $t' = a t$.
...
\begin{rem}[{\cite[\S9.1]{Brinon}}]\label{PDThickeningAcris}
	...
	hence that it is a flat \(\mathbb{Z}_{p}\)-module.
	...
	and that \(\ker A_{ \mathrm{cris} }\twoheadrightarrow O_{\mathbb{C}_K}\)
	is endowed with divided powers.
\end{rem}


\begin{rem}[]
	As seen in \cref{GKActionWR}, $\mathscr{G}_K$ acts on $W(R)$ sending $pW(R)$ to $pW(R)$
	and, thanks to \cref{PropertiesTheta}, $\ker\theta$ to $\ker\theta$.
	...
	$\ker\theta + (p)$ and argue as in diagram \eqref{eqn:ExtensionGKActionACris}
...
\begin{rem}[]\label{AcrisAlgebraStructures}
	...
	of $S$-algebra, where we recall that \(W \coloneqq W(k)\)
	and that \(S\) was defined in \cref{defn:S}.
	...
	With this structure we define the $W$-algebra morphism
	\begin{equation*}
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
		,/tikz/column 2/.append style={anchor=base west}]
	...
	\begin{tikzcd}
		...
		\sum_{n \in \mathbb{N} }^{  } F(a_n) u^{np}
		\arrow[r, "\alpha"', mapsto] &
		\sum_{n \in \mathbb{N} }^{  } F(a_n) \left[ \boldsymbol\pi \right]^{np}
	,\end{tikzcd}
	\end{equation*}
	where \(F\) denotes the Frobenius on Witt vectors.
	Recall that, as introduced in \cref{Sec:ClassificationBT/OK},
	we will denote by \(E(u) \in W[u]\) 
	the minimal polynomial of \(\pi\).
	...
	Then, by universal property of divided powers envelope,
	the morphism $W[u] \to \mathcal{D}_{ W(R) }(\ker \theta) \subset A_{\mathrm{cris}}$
	induces a morphism $\mathcal{D}_{W[u]}(E(u)) \to \mathcal{D}_{ W(R) }(\ker \theta)$.
...
\begin{rem}[]\leavevmode\vspace{-.2\baselineskip}
\begin{enumerate}\label{PropertiesBcris}
...
\item\label{GoodDefinitionExpAcris} From the above we obtain that \(\nu_p(t) \geq 1/(p-1)\).
	Then, since \(n!\gamma_n(t) = t^n\) for all \(n\),
	we can give an estimate to the \(p\)-adic valuation of
	its divided powers. Indeed we obtain
	\begin{equation*}
		\nu_p(\gamma_n(t)) = \nu_p(t^n) -\nu_p(n!) \geq
		\frac{ n - (n - s_p(n) ) }{ p - 1 } = \frac{ s_p(n) }{ p-1 }
	,\end{equation*}
	which implies that \(\gamma_n(t)\) \(p\)-adically
	go to \(0\).
...
\begin{prop}[{\cite[Proposition 9.1.6]{Brinon}}]
	...
	where we recall that \(K_0\) denotes the maximal
	unramified subextension of \(K/\mathbb{Q}_p\).
\end{prop}
...
\begin{defn}[Formal divided power thickening]
	...
	A \emph{$\mathfrak{p}$-adic divided power \(A\)-thickening of $V$}
	...
	Morphisms between divided power \(A\)-thickenings are divided power morphisms
	making the obvious diagram commute.
	If the category of $\mathfrak{p}$-adic divided power $A$-thickenings of $V$,
	...
	An initial object in the category of formal $\mathfrak{p}$-adic divided power $A$-thickenings
...
\end{prop}
\begin{proof}
	Thanks to \cref{PDThickeningAcris} 
	it is clear that $A_{\mathrm{cris}}$ is a
	...
	and \(p\) in $\mathcal{D}_{W(R)}(\ker\theta)$.
...
%	we have $y \in J$ and $d_1^p - d_2^p \in pJ$.
	...
	Then, being $D$ a separated topological ring (hence sum is continuous),
...
of our desired comparison morphism and some related ones.
...
\begin{rem}[]\label{BaseChangeAcris}
	\Cref{PDExtendCompletion} states that
	$A_{\mathrm{cris}} \twoheadrightarrow O_{\mathbb{C}_K}$
	is a divided power thickening.
	We will then need to compute, for all \(G \in \mathsf{BT}/O_K\),
	the evaluation $\mathbb{D}^*(G_{O_{\mathbb{C}_K}})(A_{\mathrm{cris}})$.
	Having already computed it, we wish to
	reduce its computation to that of $\mathbb{D}^*(G)(S)$,
	where we recalled that \(S\) was defined in \cref{defn:S}.
	Indeed, as seen in \cref{AcrisAlgebraStructures}, the \(S\)-algebra
	structure of \(A_{ \mathrm{cris} }\) is a divided power morphism
	which makes the following diagram commute
	\begin{equation*}
	\begin{tikzcd}
		O_K \arrow[d, "", rightarrow] &
		S \arrow[l, "", twoheadrightarrow] 
		\arrow[d, "", rightarrow] \\
		O_{\mathbb{C}_K} &
		A_{ \mathrm{cris} } \arrow[l, "", twoheadrightarrow] 
	.\end{tikzcd}
	\end{equation*}
	Then, calling \(f \coloneqq \spec(O_K \to O_{\mathbb{C}_K})\),
	we can invoke \cref{rem:RigidityEvaluationCrystals} and obtain that
	\begin{equation*}
...
%	Then we can evaluate the above at $A_{\mathrm{cris}}$ and obtain
	...
	.\end{tikzcd}
	\end{equation*} 
	We use \cref{BaseChangeAcris} to reduce these evaluations
	to the modules $M(G) = \mathbb{D}^*(G)(S)$ of \cref{prop:A6Kisin}.
	In particular, from \cref{DieudonneModulesSimpleExamples}, we get
	$\mathbb{D}^*(\underline{\mathbb{Q}_p/\mathbb{Z}_{p}})(S) = S \frac{ \mathrm{d}T }{ T }$.
	Then we have
	\begin{equation*}
	\mathbb{D}^*(\underline{\mathbb{Q}_p/\mathbb{Z}_{p}})(A_{\mathrm{cris}})
	...
	.\end{equation*}
	Recalling that $A_{\mathrm{cris}}$ has an $\mathbb{Z}_{p}$-algebra structure
	we obtain a pairing
	\begin{equation*}
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
	...
	As a consequence, since \(A_{ \mathrm{cris} }\) is a flat
	\(\mathbb{Z}_{p}\)-module, we have a canonical isomorphism
	\begin{equation*}
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
		,/tikz/column 2/.append style={anchor=base west}]
...
\begin{proof}
	...
	and then to reduce, using functoriality of $\rho_G$, the general case to this one.
	...
	and, upon inspecting the various actions at each step, it turns out
	that each is also $\mathscr{G}_{K_\infty}$-equivariant.

	Let's now concentrate on the particular case, so let's fix $G = \mathbb{G}_m(p)$.
	We need to explicitly compute the morphism $\rho_G$, using Messing's theory,
	in order to prove that $t$ kills $\coker \rho_G$.
	To achieve this result, let's recall that, since \(O_K\) is an
	admissible ring with ideal of definition \((p)\),
	we can invoke \cref{lem:BT/admissible} and consider a Barsotti-Tate group
	\(G \in \mathsf{BT}/O_K\) as a family \(\left\{ G_{ n } \right\}_{ n \in \mathbb{N} }\)	
	of Barsotti-Tate groups such that \(G_n \in \mathsf{BT}/ (O_K/ p^n O_K)\).
	Since this equivalence is compatible with extensions
	it will allow us to construct universal extensions and morphisms
	between them using Messing's theory.
	Indeed in the following, when invoking results from
	\cref{sec:CrystalsAssociatedToBTGroups},
	we will implicitly carry out the computations on the various \(G_n\)s.
	Since the construction of the universal extension 
	commutes with base change, by \cref{UnivExtensionCommutesWBaseChange},
	we obtain a compatible system as in \cref{lem:BT/admissible},
	hence a universal extension for our starting groups.
	Moreover the same applies to morphisms between extensions
	by \cref{thm:M4.2.4.4}.

	It's now time to start with the explicit computations.
	...
	\begin{tikzcd}[row sep = 0ex
		...
		\arrow[r, "", rightarrow] &
		\mathbb{G}_m(p)(O_{\mathbb{C}_K}) \\
		\frac{ 1 }{ p^n } \arrow[r, "", mapsto] & 
		\boldsymbol\varepsilon^{(n)}
	,\end{tikzcd}
	\end{equation*} 
	where $\mu_{ p^\infty }(O_{\mathbb{C}_K}) = \mathbb{G}_m(p)(O_{\mathbb{C}_K})$ 
	is the multiplicative group
	of all $p$-power roots of unity in $O_{\mathbb{C}_K}$.
	...
	Recalling the computations of \cref{DieudonneModulesSimpleExamples}
	we obtain the universal extension
	\begin{equation*}
	\begin{tikzcd}
	...
	\end{equation*}
	for $\mathbb{G}_m(p)$.
	For this last recall that \(\underline{\mathbb{Q}_p/\mathbb{Z}_{p}}\),
	the dual of $\mathbb{G}_m(p)$, is étale, hence $\omega_{G^D} = 0$.
	...
	for $x \in \mathbb{Q}_p/\mathbb{Z}_{p}$ and \(\boldsymbol\varepsilon\)
	our fixed system of compatible \(p^n\)th roots of unity.
	...
	Let's recall that, as seen in \cref{PDThickeningAcris}, we have a surjective homomorphism 
	\(A_{ \mathrm{cris} } \twoheadrightarrow O_{\mathbb{C}_K}\)
	whose kernel is endowed with divided powers.
	Then \cref{thm:UniqueLiftingCrystals}, thanks to
	the fact that also $A_{\mathrm{cris}}$ is $p$-adically complete,
	...
	\end{tikzcd}
	\end{equation*} 
	such that $\left.-v\right|_{ 
	\underline{\mathcal{V}}(\underline{\mathbb{Q}_p/\mathbb{Z}_{p}}_{A_{\mathrm{cris}}}) }$ 
	...
	with respect to the $p$-adic topology which, 
	thanks to \cref{GoodDefinitionExpAcris} of \cref{PropertiesBcris},
	implies that the above map is well defined.
	Moreover, since $t \in \ker \theta$, ideal with divided powers, we obtain that
	$\exp(-at) \in 1 + \ker\theta$.
	As a consequence we obtain that $\theta(\widetilde{v}(a,x)) = \boldsymbol\varepsilon^x$,
	...
	And this is exactly the required explicit construction of $\rho_G$.
	In fact, looking at the construction of \cref{constr:ComparisonMorphism},
	we see that the morphism \(\rho_G\) acts as
	\begin{equation*}
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
		,/tikz/column 2/.append style={anchor=base west}]
		\rho_G\colon A_{ \mathrm{cris} } \otimes_S \mathbb{D}^*(G)(S)
		\arrow[r, "", rightarrow] &
		\Hom_{ \mathbb{Z}_{p} }\left( T_p(G), A_{ \mathrm{cris} } \right) \\
		a \otimes b \arrow[r, "", mapsto] & 
		\left( f \mapsto \mathbb{D}^*(f)(A_{ \mathrm{cris} })(a \otimes b) \right)
	.\end{tikzcd}
	\end{equation*} 
	Moreover, in our case, \(T_p(G)^\vee \simeq \mathbb{Z}_{p} \cdot t^{-1}\),
	where the isomorphism sends \(x \mapsto x(t) \cdot t^{-1}\),
	where \(t\) is the generator of \(T_p(G)\).
	Finally we recall that, as computed in \cref{DieudonneModulesSimpleExamples},
	\(\mathbb{D}^*(\mathbb{G}_m(p))(S) \simeq S \frac{ \mathrm{d} T }{ T }\).
	Putting it all together, the morphism \(\rho_G\) acts by
	\begin{equation*}
	\begin{tikzcd}[row sep = 0ex
		,/tikz/column 1/.append style={anchor=base east}
		,/tikz/column 2/.append style={anchor=base west}]
		\rho_G\colon 
		A_{\mathrm{cris}} \otimes_S S \frac{ \mathrm{d} T }{ T }
		\arrow[r, "", rightarrow] &
		A_{\mathrm{cris}} \otimes_{\mathbb{Z}_{p}} \mathbb{Z}_{p} t^{-1} \\
		a \otimes \frac{ \mathrm{d} T }{ T } \arrow[r, "", mapsto] & 
		-a \cdot t \otimes t^{-1}
	.\end{tikzcd}
	\end{equation*} 
	...
	is arbitrary.
	We want to explicitly construct morphisms $G_{O_{\overline{K}}} \to \mathbb{G}_m(p)$
	which will allow us to reduce the study of $\rho_G$ to the above case.
	To achieve this goal we recall that $T_p(G)^\vee = T_p(G^D)(-1) = T_p(G^D)t^{-1}$.
	...
	\begin{tikzcd}[column sep=4.0em]
		...
		\arrow[u, "\mathbb{D}^*((ty)^D)(A_{\mathrm{cris}})", rightarrow] &
		A_{\mathrm{cris}} \otimes_{\mathbb{Z}_{p}} \mathbb{Z}_{p} t^{-1}
	...
	\mathbb{Z}_{p}$ and $T_p(G^D) = T_p(G)^\vee t$.
	...
	\begin{tikzcd}[column sep=4em]
		\mathbb{D}^*((ty)^D)(A_{\mathrm{cris}})(1) 
		\arrow[r, "\rho_G", mapsto] &
		-t \otimes y \\
		1 \otimes \frac{ \mathrm{d}T }{ T }
		\arrow[u, "\mathbb{D}^*((ty)^D)(A_{\mathrm{cris}})", mapsto] 
		\arrow[r, "\rho_{\mathbb{G}_m(p)}", mapsto] &
		-t \otimes t^{-1}
		\arrow[u, "\id_{ A_{\mathrm{cris}} } \otimes T_p((ty)^D)^\vee"', mapsto] 
	.\end{tikzcd}
	\end{equation*}
	Now, since we imposed no restriction on the choice of $y \in T_p(G)^\vee$, 
	commutativity of the square tells us that \(t \cdot 
	\left( A_{ \mathrm{cris} } \otimes_{ \mathbb{Z}_{p} }  (T_p(G))^\vee \right)
	\subset \ima \rho_G\), i.e$.$ that
	$t$ kills $\coker \rho_G$.
...
\begin{rem}[]
	...
	In fact, the $S$-module structure of $A_{\mathrm{cris}}$ is induced by
...
\noindent
Localizing at \(t\), i.e$.$ base changing to $B_{\mathrm{cris}}$, the above result gives rise to
...
\begin{rem}[]
	...
	its base change to $k = O_K / \mathfrak{m}_K$ the residue field of $O_K$.
	...
	.\end{equation*}
	Though here we don't have a natural filtration.
...
\begin{rem}[]
	...
	\operatorname{Fil}^1 \mathbb{D}^*(G)(S) / 
	\left( \operatorname{Fil}^1S \otimes_W \mathbb{D}^*(G_k) \right)
	,\end{equation*}
	where \(\operatorname{Fil}^1 \mathbb{D}^*(G)(S)\)
	is defined as in \cref{lem:A2Kisin}.
	...
	\begin{align*}
		...
		\operatorname{Fil}^{ i-j } B_{\mathrm{dR}} \otimes_{W} \operatorname{Fil}^{j} D_K \right) \\
		&=
