...
\begin{enumerate}
	...
	\item if $b \in B$ generates a vector space $F \cdot b$
		stable under the action of $G$, then $b \in B^*$.
\end{enumerate}
	Since we will often be interested in the case of $p$-adic representations
	of the absolute Galois group of a finite extension $K/\mathbb{Q}_p$,
	we will often assume $F = \mathbb{Q}_p$.
	In such case we will simply write that $B$ is $G$-regular
	to mean that it is $(\mathbb{Q}_p,G)$-regular.
	Moreover we will often take $G = G_K$.
\end{defn}


\begin{rem}[]
	Notice that for any $(F,G)$-regular algebra $E$ is a field.
