...
\begin{rem}[]
	...
	by quasi-coherent modules, i.e. \cref{rem:rigidityExt}, 
...
\begin{proof}
	...
	By definition of $\theta$ the rightmost square acts on elements as
	\begin{equation*}
	\begin{tikzcd}
...
In this following section, since we will mainly be working with affine schemes,
...
\begin{rem}[{\cite[Chapter IV, \S2.5]{Messing}}]
	...
	More specifically we are going to define the main crystal
	of objects $\left(U_0 \hookrightarrow U, \delta\right) \in \mathsf{Crys}(S_0)$
	with the property that $U_0$ is affine and
	$\left.G_0\right|_{U_0}$ can be lifted to $U$.

	Moreover, by \crefrange{thm:M4.2.4.1,thm:M4.2.4.2,thm:M4.2.4.3},
	we can see that, fixing one such object of $\mathsf{Crys}(S_0)$,
	$E(G)$ is independent of the chosen lift of $\left.G_0\right|_{U_0}$.
	Moreover, given a morphism $f\colon \left(U_0 \hookrightarrow U, \delta\right) \to 
	\left(V_0 \hookrightarrow V, \gamma\right)$ in $\mathsf{Crys}(S_0)$,
	given by the diagram
	\begin{equation*}
	\begin{tikzcd}
		U_0 \arrow[r, "", hookrightarrow] &
		U\\
		V_0 \arrow[u, "", rightarrow] 
		\arrow[r, "", hookrightarrow] &
		V \arrow[u, "", rightarrow] 
	,\end{tikzcd}
	\end{equation*}
	we have $f$ an open immersion.
...
\begin{defn}[]
	In the above notation, we define the crystal $\mathbb{E}(G_0)$
	...
	In the same way we define, for any morphism $u_0\colon G_0 \to H_0$ of 
	...
	Finally, for an arbitrary morphism of schemes $f\colon T_0 \to S_0$,
...
\begin{rem}[{\cite[Chapter IV, \S2.5]{Messing}}]
	...
	\begin{tikzcd}
		U_0 \arrow[r, "", hookrightarrow] &
		U \coloneqq U_0 \coprod_{V_0} V \\
		V_0 \arrow[u, "f", rightarrow] 
		\arrow[r, "", hookrightarrow] &
		V \arrow[u, "", rightarrow] 
	.\end{tikzcd}
	\end{equation*}
	tk: then it is immediate that, for a lifting $G_U$ 
	...
	More properly these equalities should be considered as isomorphisms (tk).
