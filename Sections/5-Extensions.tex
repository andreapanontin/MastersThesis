...
Since it will be used quite a lot in the suite,
...
\begin{rem}[]
	...
	Notice that, by \cref{rem:pDivDualityBaseChange}, $G^D$ will be a lift of $G_0^D$.
...
\begin{ntt}
	Let $S$ be a scheme with $p$ locally nilpotent on it, $\mathcal{I}$
...
\begin{rem}[]
	...
	assume on specific objects of $\mathsf{Crys}(S)$, like $S_0 \hookrightarrow S$,
...
\begin{defn}[Adimissible filtration]
...
tk: the following notation is non standard. Is it okay?
\begin{defn}[]
	...
	\end{tikzcd}
	\end{equation*}
	(or more consistently to the associated Zariski sheaves on $S_0$
...
\begin{enumerate}
	\item Let $G_0$ be $\mathsf{BT}'(S_0)$.
		We define, on $S$, a Zariski sheaf of sets, denoted by $\mathscr{L}$,
		as follows.
		...
		Since these form a basis for the topology of $S$, the definition
		can be extended to a sheaf on $S$.
